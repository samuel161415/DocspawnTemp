<template>
  ...
  <div class="z-auto sticky bg-white mb-0">
    ...
    <div 
      ...
      <ul class="mt-2">
        <!-- Logo -->
        <div class="mb-7 ml-3 " :class="{'flex justify-center items-center mt-3': isCollapsed}">
          <NuxtLink to="/">
        ...
        <li 
          ...
          :key="item.title" 
          class=" w-full cursor-pointer px-3 ml-2 "
          :class="{ '': isCollapsed, 'mb-2': !isCollapsed, 'border-l-2 text-primaryPurple border-primaryPurple': baseRoute === item.route }">

          <div 
            :key="item.title" class="hover:bg-primaryPurple hover:text-white rounded-md flex text-center items-center"
            ...
            <i 
              ...
              class="text-gray-500 text-center mb-2 py-3" 
              :class="[
              item.icon, 'mr-2',
                {
      ...
      <ul>
        <!-- settings -->
        <li class="mt-7  px-4 cursor-pointer ml-2 border-t w-full  py-4 " >
          
          <div 
            class="hover:bg-primaryPurple hover:text-white  rounded-md mt-2 flex  text-center items-center"
            :class="{ '   mr-2 text-sm': isCollapsed, ' py-3 px-3': !isCollapsed, 'bg-primaryPurple': openSubMenu }"
            ...
            @click="toggle">
              ...
              <i v-if="isCollapsed" 
                class="pi pi-cog cursor-pointer text-gray-500  text-center mb-2 py-3" 
                style="font-size: 1.5rem" 
                :class="{
                    'text-white': settingHovered || openSubMenu,
                    }"></i>
        ...
        <!-- speeddial -->
        <li>
          <div class="flex mb-2" :style="{ position: 'relative', height: '350px' }">
            <Toast />
            <SpeedDial :model="items" direction="up" :transitionDelay="80" showIcon="pi pi-bars" hideIcon="pi pi-times" showData="data" >
                <template #item="{ item }">
                  <!-- Customize the rendering of each SpeedDial item -->
                  <div class=" mx-4 rounded-md flex  py-2 px-7  hover:bg-primaryPurple hover:text-white w-48 space-x-4"  @click="" >
                    <i :class="item.icon" class="text-left"></i>
                    <span>{{ item.text }}</span>
                  </div>
                </template>
            </SpeedDial>
        ...
        <!-- avatar -->
        <div 
          class="flex  mt-auto px-4 "
          :class="{'space-y-2 flex-col justify-center items-center': isCollapsed, 'space-x-2 ml-4': !isCollapsed }">
    ...
    <!-- setting popup -->
    <OverlayPanel ref="op"  style="height: 0px; width: 0;">
     <div class="pop_up absolute border border-gray-100 bg-white z-5  h-44 bottom-0 mb-5 p-4 rounded-sm w-60" :class="{ ' ml-9': isCollapsed, ' ml-48': !isCollapsed }">

        <ul class=" z-1">
          <li v-for="subItem in subItems" :key="subItem.title"
            class="rounded-lg cursor-pointer flex flex-col"
           >
            <div 
              :key="subItem.title" class=" rounded-md flex px-4 py-2"
              :class="{'bg-primaryPurple': baseRoute === subItem.route }"
