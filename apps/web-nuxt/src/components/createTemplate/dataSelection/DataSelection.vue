    ...
    <div class="w-[70vw] flex flex-col items-center justify-center">
      <div class="w-full">
        <div class="card p-fluid mt-8">
          ...
          >
            <Column v-for="(columnName, index) in templateEditorStore.datasetData.keys" :key="index" :field="columnName">
              <template #header>
                <div class="flex flex-col items-center gap-2">
                  <input type="checkbox" :value="dataSourceSelectedColumns?.includes(columnName)" :checked="dataSourceSelectedColumns?.includes(columnName)" binary variant="filled" @change="toggleColumnSelection(columnName)" />
                  <p class="font-poppins whitespace-nowrap">
                    {{ columnName }}
...
watch(dataSourceSelectedColumns, (data) => {
  ...
  isValid.value = valid
  if (data?.length > 0)
    emit('updateData', { valid, step: 2 })

  else emit('updateData', { valid, step: 2 })
})

watch(() => templateEditorStore.datasetData, (newVal) => {
  console.log('dataset data chnage', newVal)
  dataSourceFileCompleteJSON.value = newVal?.allEntries?.map((f, i) => {
    return { ...f, auto_index_by_docspawn: i + 1 }
  })
  dataSourceColumnNames.value = newVal?.keys
})
watch(() => templateEditorStore.datasetSelectedKeys, (val) => {
  console.log('selected keys', val)
  dataSourceSelectedColumns.value = val
})
...
function toggleColumnSelection(columnName) {
  console.log('dataset slected keys', templateEditorStore.datasetSelectedKeys)
  console.log('datset data', templateEditorStore?.datasetData)
  console.log('data source selected columns', dataSourceSelectedColumns.value)
  const d = dataSourceSelectedColumns.value
  console.log('toggle before', d?.length, d)
  const index = d.indexOf(columnName)

  if (index > -1)
    d.splice(index, 1) // Remove column
  else
    d.push(columnName) // Add column

  console.log('toggle after', d?.length, d)
  templateEditorStore.datasetSelectedKeys = d
}
onMounted(() => {
  ...
  dataSourceSelectedColumns.value = templateEditorStore.datasetSelectedKeys
})
</script>
