  ...
  <div class="mt-20">
    ...
    <div class="">
      ...
      >
        ...
        <Column field="design" header="Design" :header-style="{ height: '4.5rem' }">
      ...
      <Dialog v-model:visible="openAddModal" modal :style="{ width: '40rem' }">
        ...
        <div class="flex space-x-24 mt-4">
          <div class="flex items-center">
            <RadioButton v-model="selection" input-id="checked" name="status" value="checked" />
            <label for="checked" class="text-lg font-poppins text-surface-600">Checked</label>
          </div>

          <div class="flex items-center">
            <RadioButton v-model="selection" input-id="unchecked" name="status" value="unchecked" />
            <label for="unchecked" class="text-lg font-poppins text-surface-600">Unchecked</label>
          </div>
        </div>
        ...
        <div class="mt-7">
          ...
          <img v-if="fileUrl" id="output" accept="image/*" class="mt-5 object-cover h-auto w-[64px]" :src="fileUrl" />
          <!-- <div
        ...
        <div class="flex justify-center mt-10">
          <Button :disabled="!selection || !fileUrl" type="button" label="Upload" severity="success" class="w-32" @click="submitCheckbox" />
        </div>
      </Dialog>
      <!-- <Toast /> -->
...
async function submitCheckbox() {
  ...
  try {
    const response = await fetch(`${runtimeConfig.public.BASE_URL}/checkboxOptions`, {
      method: 'POST',
      headers: {
    ...
    toast.add({ severity: 'success', summary: 'Info', detail: 'Checkbox added successfully', life: 1000 })
    openAddModal.value = false
  }
  catch (error) {
    console.error('Error:', error)
    toast.add({ severity: 'error', summary: 'Info', detail: 'Unable to add the checkbox', life: 5000 })
    openAddModal.value = false
  }
//   openAddModal.value = false
}
...
async function fetchCheckboxOptions() {
  ...
  try {
    ...
    const response = await fetch(`${runtimeConfig.public.BASE_URL}/checkboxOptions/${user?.value?.email}`)
    if (!response.ok)
      throw new Error(`Network response was not ok ${response.statusText}`)
    const data = await response.json()
