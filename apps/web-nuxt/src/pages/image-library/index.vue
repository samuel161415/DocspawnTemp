  ...
  <div class="h-full w-full bg-white rounded-lg overflow-scroll no-scrollbar">
    <div class="px-8 py-8 h-full">
      <h2 class="font-semibold text-surface-600 text-2xl px-3 py-2">
        {{ $t("Cp_sidebar.image_library") }}
      </h2>
      <div class="w-full border mt-4">
        <div v-if="images?.length > 0" class="flex flex-wrap gap-4 my-4 p-2 ">
          <div v-for=" image in images" :key="image.id" class="relative border min-w-48 h-32">
            <img class="h-full w-auto" :src="image.image_url" />
