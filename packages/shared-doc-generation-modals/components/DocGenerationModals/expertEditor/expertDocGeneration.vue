<template>
  <div class="flex justify-center space-x-8 h-3/4 w-max">
    <div class=" h-full mt-0" :style="{ marginRight: '12px' }">
      ...
      <div :class="`w-full flex ${mobile ? 'justify-center' : 'justify-center'}`" :style="{ marginTop: '12px' }">
        <Button contained severity="success" :label="$t('Cp_dataToDoc_generation.spawn_documents')" class="w-max font-poppins font-normal text-[16px] leading-[25px]" :disabled=" isGeneratingDoc" @click="generateDocs" />
      </div>
    </div>
    <!-- <CanvasPreview v-if="template" :template="template" :selected-rows="selectedRows" use-case="dataToDoc" /> -->
    <div v-if="template?.id" class="w-max border border-red ">
      <EditorPreview :editor-html-content="template?.editors_data" :template="template" />
    </div>
  </div>
...
async function generateDocs() {
  ...
  try {
    const response = await fetch(`${runtimeConfig.public.BASE_URL}/generate-documents/expertEditor/${template.value?.id}`, {
      method: 'POST',
      headers: {
